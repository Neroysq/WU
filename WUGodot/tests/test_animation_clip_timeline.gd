...
func run_all() -> Dictionary:
	...
	if clip.pose_at(0.0, hu_light) == "hu_light_00" and clip.pose_at(0.09, hu_light) == "hu_light_01" and clip.pose_at(hu_active_start_t - 0.01, hu_light) == "hu_light_04" and clip.pose_at(hu_active_start_t, hu_light) == "hu_light_05" and clip.pose_at(hu_active_end_t + 0.01, hu_light) == "hu_light_10" and clip.pose_at(1.0, hu_light) == "hu_light_15":
		passed += 1
	else:
		failed += 1
	...
	if heavy_clip.pose_at(0.26, hu_heavy) == "hu_heavy_02" and heavy_clip.pose_at(heavy_active_start_t - 0.01, hu_heavy) == "hu_heavy_04" and heavy_clip.pose_at(heavy_active_start_t, hu_heavy) == "hu_heavy_05" and heavy_clip.pose_at(heavy_active_end_t, hu_heavy) == "hu_heavy_10" and not heavy_clip.use_fighter_offset and is_equal_approx(heavy_clip.sample_track("offsetX", 0.50, 0.0), 0.0) and not heavy_clip.has_track("scaleX") and heavy_clip.sample_track("smear", 0.60) > 0.5:
		passed += 1
	else:
		failed += 1
		failures.append("heavy video clip should hold readable anticipation, cleave at active start, recover at recovery start, and remain presenter-stationary")
	...
	if jump.use_fighter_offset and jump.pose_at(0.0) == "hu_jump_00" and jump.pose_at(0.5) == "hu_jump_07" and stunned.use_fighter_offset and stunned.pose_at(0.5) == "vp_stun_b":
		passed += 1
	else:
		failed += 1
