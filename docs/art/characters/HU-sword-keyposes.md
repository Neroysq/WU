...
- **SCABBARD LAW (2026-07-09, user finding):** the drawn sword and the scabbard must never read on the same side. At 256px a far-side scabbard is mostly occluded anyway, so: **combat poses show NO scabbard** (drawn-sword states); the scabbard exists only in sheathed states (relaxed stand, entry-draw start) at the LEFT hip. Prompts for drawn poses must NOT mention a scabbard; prompts for sheathed poses say "sheathed in the scabbard at his left hip." Gauntlet check: no dark sheath shape on the sword-hand side.
