...
- **Add a per-type hanzi glyph** drawn in each node and in the legend — on-brand (the game uses hanzi throughout), palette-free, instantly distinct, colorblind-safe. Suggested: Duel `斗` · Elite `精` · Ambush `伏` · Master `師` · Shop `商` · Rest `息` · Boss `王`. Draw a small centered glyph at each node (`map_scene.gd` node render ~`:64-69`) and beside each legend swatch (`_draw_node_legend` ~`:152-167`). Keep it legible at the node's 12px radius (small glyph above/over the node, or tint the inner dot + glyph in the legend only if node-size is too small — implementer's call after a capture).
- **Nudge the worst color pair within the existing palette** (don't invent colors — palette is VINIK24-constrained): make **Boss a darker/deeper crimson** so it reads distinct + ominous vs Ambush's brighter red; confirm Elite vs Shop read apart (with glyphs present, a small nudge suffices). Reuse existing `GameConstants` color constants only.
**Verify:** `./run.sh --capture {kind:ui,screen:map}` → each node type distinguishable by glyph at a glance; legend shows glyph + label + color.
...
**Verify:** `./run.sh --capture {kind:ui,screen:rest}` → locked rows show the chip + dimming, no strikethrough.
...
**Verify:** `./run.sh --shot-combat` with a built loadout (or a capture with a forced build) → equipped boons visible, HUD still readable, fight not obscured. If clutter, switch to the pip fallback.
