...
## Heavy attack 🔄 (Phase 4, Gate 1 in progress)

- **Concept:** **Overhead Cleave** — big telegraphed overhead raise → downward diagonal cleave → low heavy settle. Deliberately distinct from light's horizontal thrust (a horizontal heavy strike reads as "just a slow light").
- **Anchors (selected this session, NOT yet recorded):** guard → **windup** (cand_2, overhead) → **strike** (cand_4, cleave down) → **recover** (to be regenerated) → guard.
- **Recipe:** `--reference-seq guard windup strike recover guard`.
- **Status / pending:** Concept + windup cand_2 + strike cand_4 chosen in the 2026-06-16 Gate-1 review. **Not yet reflected in the repo** — no heavy entries in `art/keyframes/hu/` or `keyframes.manifest.json` yet; candidates live only in `/tmp`. Next: implementer records windup/strike to `art/keyframes/hu/heavy/` + manifest, regenerates **recover** via image-to-image *from strike cand_4* (inherits grip — the original recover candidates read "opposite grip", see rule below) → quick recover-only Gate-1 confirm. Then: install `vh_`, heavy timeline rewrite (markers + smear, **delete legacy offsetX/scaleX windup transforms** = motion-study transform rollback), `hu_heavy` max-extension collision + its own reach re-sync ✋ STOP (cleave tip likely *shorter* horizontal reach than light), Gate 2.
