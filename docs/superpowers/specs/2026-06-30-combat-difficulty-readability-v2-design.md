...
6. **Early enemies — readable on-ramps.** Weak pool: long, legible telegraphs, aggressive but honest, **little/no reactive blocking**. Reactive defense (reactive *blocking*, assassin teleport, spear spacing, bear gimmicks) **ramps in** at strong/elite/boss. The early game *teaches* the duel; the mind-games are the late-game test. (Enemies block but do not *parry* today — an enemy parry would be a new mechanic, scoped as a future escalation in §6, not assumed here.)

---

## 3. Levers (candidate knobs — pick after Phase-1 diagnosis)
- **Telegraph audit (both weak archetypes):** confirm the weak pool's attacks have windups generous enough to dodge-then-punish on reading alone; lengthen windup in `data/Attacks/*.json` if too fast. Cover **both**: `bandit_swordsman` → `bandit_slash`, `bandit_overhead`, `bandit_thrust_perilous`; `bandit_spearman` → `spear_long_thrust`, `spear_wide_swing`. Only `bandit_thrust_perilous` is unblockable/perilous (`is_perilous:true`, `Attacks.json:63`); the spear attacks and `bandit_slash`/`bandit_overhead` are normal **parryable, non-perilous** attacks (`Attacks.json:393,408`). All of them must be *dodgeable* on telegraph; the diagnosis confirms whether the dasher dies to a too-fast or un-dodgeable window.
- **Early enemy damage (if diagnosis shows attrition kills the dasher):** the early-pool *output* is the likely lever, applied via the per-pool-class pressure multiplier below (turned DOWN for the weak pool, UP for later pools).
...
- **Reactive-*blocking* ramp — runtime pool-class modifier, not per-archetype data.** Scope note: enemies **block, they do not parry** (`combat_system.gd:238` sets only `is_blocking`; `test_enemy_block_no_parry.gd`). So this ramps reactive *blocking* (`blockChance`), not a "block→parry." `blockChance` lives per enemy JSON, but `sect_disciple` appears in **both** `strong_pool` and `elite_pool` (`DifficultyCurve.json:6`), so one JSON value can't be both tiers — implement the ramp as a **per-pool-class `blockChance` override applied at encounter setup** (same runtime-modifier path as the pressure multiplier below), e.g. weak→0, strong→~0.12, elite→~0.25, boss→authored. (Alternatives rejected: split archetypes = data duplication; per-encounter JSON overrides = scattered.) A late-game enemy **parry** mechanic is a deliberate future escalation — **out of scope** here (§6).
- **Rising ramp / attrition — per-pool-class pressure multiplier with an explicit application point + plumbing.** Add a **`pressure_by_pool_class`** map to `DifficultyCurve.json` keyed by `pool_class` (weak/strong/elite/boss) — NOT by map tier, since strong and elite can occur at the same `node.tier`. **Apply it to the HitContext when the attacker is an enemy** — i.e. multiply `ctx.hp_damage` and `ctx.posture_damage` in `resolve_hits` (`combat_system.gd:368-382` region, after the technique/block math) by the enemy's `incoming_pressure_mult`. **Do NOT rely on `enemy.attack_damage`/`attack_posture_damage` (`enemy_factory.gd:33-34)** — pattern attacks use `attack_def.damage`/`attack_def.posture_damage` (`combat_system.gd:368`), so scaling the fighter fields would be a no-op. Store `incoming_pressure_mult` on the enemy `Fighter` at spawn, set from the encounter's **`pool_class`**. **Plumbing (required):** `EncounterResolver.begin_encounter` returns `pool_class`/`normal_combat_ordinal`/`ambush_wave`/`node_id` — **not `tier`** (tier is on `node.tier`, available separately, `encounter_resolver.gd:22`). But `main.gd:210` passes only `archetype` into `setup_combat`, and `CombatSetup.prepare` takes only `forced_archetype` (`combat_setup.gd:6`). So thread the **modifier context** (`pool_class` — plus `node.tier` only if a per-map-tier nuance is ever wanted — or the precomputed mult + blockChance override) from the encounter dict through **all three setup paths** — `CombatScene.setup_combat` → `CombatSetup.prepare`, and the equivalent `CombatSim`/`CombatController` paths — and apply it to the enemy immediately after creation in each. (Optionally also scale enemy `postureMax`/`healthMax` at spawn — that path *does* read fighter fields.) Composition levers are data-only: `weak_count`, `archetype_rank`, `node_type_weights_by_tier`.
...
- `facetank` win ≈ **0.00**.
- **Zero timeouts** in every batch — count `combats[].timed_out` (or `combats[].winner == "timeout"`) across `transcripts[]`, **NOT** `transcript.outcome` (which is only `victory`/`defeat`, `run_driver.gd:41`; combat timeouts live per-combat in `combat_result.gd`).
...
2. **Rising ramp + attrition + reactive-defense ramp** — add the per-pool-class pressure multiplier (`DifficultyCurve.json` + the `incoming_pressure_mult` application point in `resolve_hits`, §3), the per-pool-class `blockChance` runtime override at encounter setup, and composition tuning. **Gate:** competent pre-boss fight win ~0.65–0.75; deaths shift to the back half; lethality late; checker still accepts (monotonic, boss-highest, tier1<20%).
...
4. **Record** — before/after table, the per-pool-class pressure values, and the note that this calibrates the "Normal" difficulty tier (feeds the deferred difficulty-selection slice).
...
- **Enemy *parry* mechanic** — enemies block but don't parry today; a late-game enemy parry (a real escalation) is a deliberate future slice, not built here.

## 7. Components
- **Data:** `data/Enemies/*.json` (`blockChance`, possibly `aggression`/HP/posture), `data/Attacks/*.json` (weak-attack windups), `data/Difficulty/DifficultyCurve.json` (`pressure_by_pool_class` + per-pool-class `blockChance` override + composition).
- **Code (for the runtime modifiers):** a new `incoming_pressure_mult` field on `Fighter` (default 1.0); `enemy_factory.gd`/encounter setup sets it + the per-pool-class `blockChance` override from the encounter `pool_class` (curve via `data_manager.gd get_difficulty_curve`); `combat_system.gd resolve_hits` (~:368-382) multiplies `ctx.hp_damage`/`ctx.posture_damage` by the enemy attacker's `incoming_pressure_mult`. Characterization test that an enemy's pressure mult scales dealt damage but the player's stays 1.0.
