...
**Date:** 2026-06-17
**Status:** ✅ SHIPPED (builds 4 `b4187d4` · 5 `6c1a113` · 6 `712261e`, 2026-06-18, playtest-approved)

> **Build 6 (final size re-tune, playtest-driven).** Static head-matching previews read uniform, but **in-game** walk + all held + entry were significantly oversized — head-size ≠ overall-character-size when head/body proportions vary across the generated poses, and build-5's hit check compared hit to *block* (itself oversized) rather than to idle. Fixed by **eyeball-calibrating overall character size against idle** via an interactive slider tool: walk 0.728, entry 0.968, held per-pose 0.53–0.76 (idle/light/heavy unchanged). A foot-anchor regression (down-scaling shrank the canvas but `footAnchor.y` kept its stale pre-scale value → characters floated) was fixed in `measure_anchors`/`anchor_sanity` (now rejects/guards foot anchors outside the canvas). Lesson: **judge size by overall character against idle, in motion — not by static head boxes.** Balance frozen throughout.

> **✅ COMPLETE.** Hu's 229 `v*` poses normalized: per-action head-size constants (idle 1.00 ref · light/heavy 1.06 · walk 1.04 · entry 1.08), drift removed, feet planted (except airborne/dash), light lunge moved to presenter travel. **Balance frozen** — combat uses `range_units` (unchanged), so the size pass is purely visual; the reach comparator's "out-of-band" was geometric-only (the drawn blade extends ~5% past the hitbox, a benign visual gap). Detector journey: geometric → occlusion-robust geometric → vision/face-color (build 3) → metric fix to head-area-on-guard-frame (build 4). **Build 5** fixed the held poses: they are independent keyframes (not one ratio-maintaining clip), so build-4's single held constant (1.04) left the bigger keyframes oversized (hit read ~1.3× the others). Re-done per-pose via manual head-core boxes (auto-detection marks all held occluded) → hit 0.825, others ~1.0. Four flaws caught at the review gate by eye: vh_028 height contamination, width under-correction, stale build-3 overlays on build-4 sprites, and the held group-constant oversizing hit. Deferred: blade-length frame variance (pre-existing); entry foot-y polish.
...
### Build 3 re-review (2026-06-17)

Build 3 (provider `local_face_color_component_bootstrap` — a local skin/face-color heuristic + alpha-snap, deterministic; not an LLM call) **fixed the detection**: every checked box sits on the face, the 3 truly-occluded windup frames (vh_022/025/028) are held out, no blade contamination. **But the size METRIC is wrong.** Constants were derived from head **width**, the least size-sensitive dimension. Guard-pose comparison (the upright pose shared by every action, so directly comparable to idle): light/heavy guard heads are **0.90 of idle by area** (0.94 by height) but only **0.96 by width** — so the width-median under-corrected to 1.011/1.022 when ~1.05 was needed, leaving light/heavy heads visibly smaller (caught by eye). Walk also scaled the wrong way (0.979 on a head already 0.92 of idle).
