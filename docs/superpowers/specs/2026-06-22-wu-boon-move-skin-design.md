...
- **`FighterPresenter.set_move_skins(slot_school_map: Dictionary)`** (slot → school id), called at **combat setup** from the player's `BoonLoadout` (the loadout is fixed for the duration of a combat, so it is set once per fight). On set, the presenter lazy-loads the referenced schools' overlay clips + manifest poses and caches them, and records the slot→school map.
- **Clip resolution becomes skin-aware:** where the presenter currently uses `clip_for(state)`, it first maps state→slot; if that slot is infused by school `S` and a variant clip is registered for `(S, state)`, it uses the variant clip id; otherwise the base clip id. `handles_state` accepts variant clips too.
- **Recolor + stance tint** go through the presenter's existing shader material (`_mat_current`), layered **under** the telegraph/active-flash signals (`set_flash`) so combat readability is never degraded. The same tint mechanism serves both the un-arted-slot fallback and the active-stance recolor.
...
- **Un-arted infused move slot** (e.g. a Venom block before a Venom block clip exists, or any non-Venom school today): play the **base clip tinted toward that school's `themeColor`** via the presenter material — a subtle, readable tint under the telegraph/active-flash signals. Every school reads as *something* immediately; the bespoke variant clip silently supersedes the recolor when it lands.
...
3. **Recolor + stance tint:** `themeColor` tint via the presenter material for infused-but-unskinned slots and for active stance, layered under telegraph/flash. Tests: recolor flag + readability + stance activate/deactivate.
