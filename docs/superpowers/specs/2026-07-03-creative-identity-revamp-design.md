...
**Method: environmental + boss text beats** (written dialogue on the boss-beat surface — no voice acting implied). No cutscenes, no hub, no run-memory system.
...
- **Immortal dialogue — text only (no VO), scoped to what exists.** Today the only fightable immortal is Xiong Tie, and the only text surface is his **death beat** (`_show_boss_beat("破山!")` on boss death, `combat_scene.gd:496/:961` — there is no intro text surface, only an entry animation). The identity pass: **(a) rewrites the death-beat/outro strings** in the new fiction, and **(b) adds one small hardcoded intro text beat** at boss-fight start, reusing the boss-beat renderer — **which needs a dynamic caption path first**: today it always draws the death caption ("Iron Bear falls," `combat_scene.gd:961`), so calling it at fight start would show the death subtitle. Extend it to `func _show_boss_beat(message: String, duration: float = 1.1, caption: String = "") -> void` — keeping the current second arg so the existing call `_show_boss_beat("破山!", 1.1)` (`combat_scene.gd:496`) stays valid; the death beat passes a caption, the intro passes none (empty = no caption drawn). Explicitly in scope, still no dialogue system. A **per-immortal pre/post-fight dialogue schema** (data-driven lines) is deferred until the other immortals' fights exist; the six personalities live in this spec + school flavor text meanwhile.
