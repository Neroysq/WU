...
- **Immortal dialogue — text only (no VO), scoped to what exists.** Today the only fightable immortal is Xiong Tie, and the only text surface is his **death beat** (`_show_boss_beat("破山!")` on boss death, `combat_scene.gd:496/:961` — there is no intro text surface, only an entry animation). The identity pass: **(a) rewrites the death-beat/outro strings** in the new fiction, and **(b) adds one small hardcoded intro text beat** at boss-fight start (reusing the existing boss-beat renderer) so the gatekeeper speaks before the fight — explicitly in scope, still no dialogue system. A **per-immortal pre/post-fight dialogue schema** (data-driven lines) is deferred until the other immortals' fights exist; the six personalities live in this spec + school flavor text meanwhile.
