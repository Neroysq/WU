...
- [ ] **Step 6: Verify** — `./run.sh --test` 0-failed; then **dogfood with a forced high tier.** Note (reviewer P2): `main.gd _prepare_capture_ui` ignores a top-level `tier` — it only honors a non-empty `spec.payload` (else it generates a normal offer that may roll `common`). So either **(a)** pass an explicit `payload` matching `RunFlow.generate_boon_offer_payload`'s shape with each offer's `"tier":"legendary"` plus one duo offer: `{"kind":"ui","screen":"boon_offer","payload":{"school":"venom","offers":[{"boon_id":"venom_light","tier":"legendary"},{"boon_id":"<duo_id>","tier":"legendary"}, …]}}`, **or (b)** add a small `forced_offers:[{boon_id,tier}]` spec key the handler expands into a payload (do this if the raw payload shape is awkward to hand-author). Capture and eyeball: card shows name + the *full cumulative* rider text, the duo reads "Duo · …" (not "Epic Duo"), nothing overflows — `assert_nonblank` + visual check.
