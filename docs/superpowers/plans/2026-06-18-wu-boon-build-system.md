...
**Files:** Create `WUGodot/scripts/boons/boon_text.gd`; Modify `WUGodot/scripts/scenes/boon_offer_scene.gd` (`_offer_label`/`_offer_description`), `WUGodot/scripts/scenes/loadout_view.gd` (row summary), `WUGodot/data/Boons/Boons.json` (add `name`); Test `WUGodot/tests/test_boon_text.gd`.

- [ ] **Step 1: Failing tests** — (a) `BoonText.describe(get_boon("venom_light"), "epic")` mentions the **base venom AND the rare+epic riders** (slow, spread), ≠ "Adds move effects."; at `"common"` only the base; (b) `BoonText.name(boon)` returns the readable `name` (not the raw id); (c) **effect-type coverage gate:** every `effect.type`/rider `type` in `Boons.json` has a describer template (no generic fallback); (d) **label rule (reviewer P1):** for a duo/mastery boon the offer label uses the **kind** ("Duo · …" / "Mastery · …"), never a rolled rarity ("Epic Duo"); move/passive labels use rarity; (e) **name content gate (reviewer P2):** every boon in `DataManager.get_all_boons()` has a non-empty `name` with **no underscore / no raw-id** (so new boons can't silently regress to humanized ids); (f) **`summary` stays short (reviewer P2):** `BoonText.summary(get_boon("venom_light"), "legendary").length() <= 48` and it is NOT equal to `describe(...)` / does not contain all cumulative rider clauses (the loadout-overflow fix depends on summary staying brief).
...
- [ ] **Step 3: Implement** `boon_text.gd`:
  ...
  - `static func summary(boon, tier) -> String` — a **SHORT** one-liner for the loadout row (base-effect clause only, or name + rider count); `describe` stays the full cumulative text (offer cards only). Loadout rows are one line at font 11 — full text overflows (reviewer P2).
  - `static func label(boon, tier) -> String` — move/passive → `"<Tier> · <name>"`; **duo/mastery → `"Duo · <name>"` / `"Mastery · <name>"`** (no rarity — they're flat; `_roll_tier` runs for all offers but must not surface on duo/mastery).
...
- [ ] **Step 4: Wire UI** — `boon_offer_scene.gd`: `_offer_label(offer)` → `BoonText.label(boon, offer.tier)` (handles the move/passive-rarity vs duo/mastery-kind rule); **`_offer_description(offer)`** (the real fn — reviewer P3, not `_offer_n`) → `BoonText.describe(boon, offer.tier)`, replacing the `"Adds %s effects."` line. `loadout_view.gd`: replace the row `hint`/`_effect_summary` (the one-line font-11 surface) with `BoonText.summary(boon, tier)` — NOT the full `describe` (overflow). A hover/detail tooltip showing full `describe` is a later nice-to-have.
- [ ] **Step 5: Content** — add a readable `name` to each boon in `Boons.json` (and any `desc` overrides where the template reads awkwardly).
- [ ] **Step 6: Verify** — `./run.sh --test` 0-failed; then **dogfood with a forced high tier.** Note (reviewer P2): `main.gd _prepare_capture_ui` only honors a non-empty `spec.payload` (top-level `tier` ignored; an empty payload rolls a normal — possibly `common` — offer), and `BoonOfferScene` **draws `offer["boon"]` (the full boon dict)**, not `boon_id`. A `{boon_id,tier}`-only payload renders nothing. **So add a required `forced_offers:[{boon_id,tier}]` spec key** to `_prepare_capture_ui`, which expands each into a full offer (`{boon_id, "boon": DataManager.get_boon(boon_id), tier}`) and sets `payload.offers` (+ `payload.school`). (Hand-authoring full `boon` objects in the spec is error-prone — `forced_offers` is the supported path.) Capture a spec with a `legendary` venom move-boon + one duo, and eyeball: card shows name + the *full cumulative* rider text, the duo reads "Duo · …" (not "Epic Duo"), nothing overflows — `assert_nonblank` + visual check.
