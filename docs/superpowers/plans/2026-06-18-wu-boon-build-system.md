...
- [ ] **Step 6: Verify** — `./run.sh --test` 0-failed; then **dogfood the harness with a forced high tier** (a generic spec can roll `common` at low depth and won't exercise cumulative riders or wrapping — reviewer P2): capture a ui spec with an explicit **`tier: "legendary"`** venom offer (and a duo offer), and eyeball that the card shows the name + the *full cumulative* rider text, the duo reads "Duo · …" (not "Epic Duo"), and nothing overflows — via `assert_nonblank` + visual check.
