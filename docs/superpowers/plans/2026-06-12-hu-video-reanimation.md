...
3. **Video.** `animate-video --reference-seq` with approved keyframes in order (loops repeat the first as last). Check `contact_sheet.png` + `preview.gif` in the run dir: static camera, no identity drift. Reject and re-run (new seed) on camera drift — do not hand-fix frames. **Prompt lessons (from the walk experiment):** frame motion in the game's own genre terms — "2D fighting game" / "Street Fighter" framing produces grounded combat motion; words like "march / high knees" produce stiff military motion. Always add "stepping/moving in place without translating across the frame" (the presenter handles travel) and the relevant negatives ("NO deep lunges" etc.). **For LOOPS specifically:** pinning the same harvested pose at BOTH ends (`--start-frame`/`--end-frame` = one master frame) gives begin≡end closure by construction — more reliable than trimming a free-end run, whose gait wanders. If pinned output looks static, push the prompt for MORE motion between the endpoints ("two full steps") rather than unpinning.
4. **Selection.** List candidate frames by measuring tips/feet (AnchorMeasure probe or visual scan of masters). Select by pose progress, dense where motion supports it (light-attack precedent: ~30 frames). Skip glitch frames (object flicker). For loops, verify first≈last selected frame. **Preview caveat:** judge green/spill on the actual master PNGs or final vinik24 pixels, NOT on downscaled low-color GIF previews — aggressive GIF palettes bleed saturated colors (e.g. a green scabbard) into edges and fake "green outlines" that aren't in the source (measure edge pixels to confirm; despill auto-runs inside animate-video).
...
**Implementation note (2026-06-12):** the installed `aiexp sprite-extractor` exposes `animate-video --reference-seq` and `pixelize --fit-mode exact`, but not `rawgen`. For the first Guard Gate 1 pass, candidates were generated serially through the current codex-backed still path:

```bash
aiexp sprite-extractor extract \
  --photo ~/GitReps/AIexp/experiments/video-animation-spike/runs/hu-refs-seq/masters/master_001.png \
  --describe "side view, full body, young wuxia swordsman in deep blue robes, combat guard stance, sword DRAWN and held ready in front, empty scabbard visible at his hip, exaggerated confident wide stance, comical wild energy, clean white background" \
  --backend codex --codex-timeout-sec 600 --bg-color '#00FF00' \
  --output-dir /tmp/wu-reanim/keyframes-raw/guard-N
cp /tmp/wu-reanim/keyframes-raw/guard-N/extract/character.png \
   /tmp/wu-reanim/keyframes/guard/stance/cand_N.png
```

This preserves the Gate 1 review flow, but the outputs are pixel-styled stills rather than the originally planned smooth raw masters. If Gate 1 approves one, the idle-video step can still consume it as `--reference-seq`; if the pixel-styled reference hurts video quality, the follow-up is to expose/restore a true `rawgen` CLI before generating breath.
...
**RESOLVED BY EXPERIMENT (2026-06-13, runs 1–4 + user Gate verdict).** The keyframe-authoring approach below was superseded. Findings:

- **Locomotion needs NO authored keyframes** — Seedance free-generates a usable walk from the guard reference alone, *if the prompt is genre-correct*.
- **Prompt is decisive.** "march / lift each knee / high knees" → stiff military lunges (run1, rejected). Fighting-game framing → real grounded steps: *"2D fighting game neutral walk, low grounded combat stance, small measured forward steps, feet low to the ground, stalking like a Street Fighter character, NO high knees, NO deep lunges, steady body height, stepping in place without translating across the frame."*
- **The loop/motion tradeoff:** free end (`--start-frame guard` only) → real motion but the gait *wanders*, so no window loops cleanly (best seam ~156, still hitches — the "abrupt end"). Pinning the **same stride pose at both ends** (`--start-frame`/`--end-frame` = one harvested master frame) guarantees begin≡end loop closure. With "one stride / careful advance" the between-motion is a subtle poised in-stance weight-shift (**run3 — user-selected as best**: cleanest loop, reads as a careful combat advance); with "two full alternating steps" the motion is much larger while still looping (run4).
- **Anchor harvesting:** the pinned stride pose was copied straight from run2's `master_037.png` — no hand authoring. `/tmp/wu-reanim/walk_anchor.png`.
- **Green outlines were a preview artifact** — downscaled low-color GIFs bleed the green scabbard into edges. Measured 0% edge spill; despill auto-runs inside `animate-video`. The scabbard + jade tassel are canon Hu (user: keep green).

**Chosen source: run3** (`/tmp/wu-reanim/walk-run3/walk/`), pinned both ends to `walk_anchor.png`, "careful one-stride" motion. To reproduce if /tmp is cleared:
```bash
aiexp sprite-extractor animate-video --output-dir /tmp/wu-reanim/walk-run3 --action walk \
  --motion "side view, stationary locked camera, 2D fighting game neutral walk: a young wuxia swordsman advances carefully in a low grounded combat stance, sword held ready, small measured forward steps with feet low to the ground, controlled balanced weight, stalking like a Street Fighter character, NO high knees, NO deep lunges, steady body height, completes exactly ONE full stride and returns precisely to the starting stance for a seamless loop, stepping in place without translating across the frame, full body visible" \
  --start-frame /tmp/wu-reanim/walk_anchor.png --end-frame /tmp/wu-reanim/walk_anchor.png --duration 4
```

- [ ] **Step 1: Select the loop window** — run3 is pinned, so use the full clip frames **2–96** (skip frame 1 = literal composite). It loops by construction (begin≡end anchor).
- [ ] **Step 2: Stage + normalize + pixelize** per Runbook 5–6: copy frames 2–96 (+ sidecars) renumbered 001..N, plus the canonical idle reference (`/tmp/wu-va-run3/idle/masters/master_001.*`); `./run.sh --scale-masters`; `pixelize --fit-mode exact --palette vinik24` at the printed out-size. **Density check:** confirm the pixelized character content-height matches the installed `vi_`/`va_` family (~177px) — animate-video's char-height-frac framing can yield a smaller character; if so, adjust out-size/scale so on-screen size matches idle (this is the bandit-pilot "smaller in frame" risk).
- [ ] **Step 3: Install** — `./run.sh --install-video /tmp/wu-walk-final --action=walk --prefix=vw --frames=<labels>` then `./run.sh --import`.
...
- [ ] **Step 5: Gates; `--shot-action WALKING`; FOOT-SLIDE is the key check.** run3's legs are deliberately subtle, so this is the real risk: translate Hu at `move_speed` and watch feet vs ground. If they slide, slow `move_speed` (a poised advance *should* be slow) or tune clip `duration` so the cycle matches travel — show the math in the report. Do NOT regenerate for this; it's a timing tune.
